...
with the paths in kg per square metre.  Cloud fraction is deliberately
not used as a multiplier: the condensate paths are already grid-box
means, so dividing by fraction would give an in-cloud depth, whereas
what a satellite link experiences over a quarter-degree cell is the
mean.

TRANSMITTANCE.  As in the existing Beer-Lambert model,

    W(theta) = exp( -(tau_clear + tau_cloud) / sin(theta) )

with tau_clear = 0.30 at 800 nm.  The zenith value is reported here;
the simulator applies the elevation term per pass.
...
def main() -> None:
    ...
    for name in sorted(groups):
        ...
        df["station"] = name
        rows.append(df[["station", "time", "tcc", "tau", "W_zenith"]])
    ...
    print(f"{'station':16s} {'hours':>6s} {'mean tcc':>9s} {'med tau':>8s} "
          f"{'P(W>0.1)':>9s} {'lag-1 r':>8s} {'decorr h':>9s}")
    for name, s in series.items():
        ...
        print(f"{name:16s} {len(v):6d} {sub.tcc.mean():9.3f} "
              f"{np.median(v):8.3f} "
              f"{(sub.W_zenith > 0.1).mean():9.3f} {r1:8.3f} "
