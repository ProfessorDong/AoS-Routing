...
# five weather realizations, matching every other figure
SEEDS, HORIZON = [0, 1, 2, 3, 4], 43200
...
def _one(args):
    s, th, pol, ls = args
    return A.run(A.TqdConfig(horizon_s=HORIZON, seed=s, theta=th,
    ...
    for th in (0.25, 0.5, 0.75):
        for pol in ("aos_tqd", "fungible"):
            with Pool(len(SEEDS)) as pool:
                r = pool.map(_one, [(s, th, pol, ls) for s in SEEDS])
            m = lambda k: float(np.mean([x[k] for x in r]))
            sd = lambda k: float(np.std([x[k] for x in r]))
